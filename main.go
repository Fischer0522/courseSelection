...
func sendRequest(newReader string,newRequestUrl string,cookie Cookie) []byte{
	client := &http.Client{}
	...
	req.Header.Set("Cookie", "JSESSIONID="+cookie.JSESS+"; BIGipServerp_new_hr_-_authserver.cumt.edu.cn="+cookie.BIGipServer+"; TWFID="+cookie.TWFID)
...
func searchClass(cookie Cookie) []SearchResult {
	searchList := make([]SearchResult,0)
		newReader := "yl_list%5B0%5D=1" +
			"&rwlx=2" +
			"&xkly=0" +
			"&bklx_id=0" +
			"&sfkkjyxdxnxq=0" +
			...
			"&jg_id=04" +
			"&njdm_id_1=2020" +
			"&zyh_id_1=A737C8D6D90A009EE053C0A86D5D8E09" +
...
func getDetail(kch_id string,cookie Cookie) []DetailResult {
	content := "rwlx=2" +
		"&xkly=0" +
		"&bklx_id=0" +
		"&sfkkjyxdxnxq=0" +
		...
		"&kch_id="+kch_id +
		"&jxbzcxskg=0" +
		"&xkkz_id=EF9E3D0F99E56F21E0531F70A8C0CC18" +
		"&cxbj=0" +
		"&fxbj=0"
...
func chooseClass(jxb_ids string,kch_id string,kcmc string,cookie Cookie) {
...
func doTaskWithGoRoutines(cookie Cookie){
	...
	for i := 0;i < 3;i++{
		...
		go func(cookie Cookie) {
			ls :=  searchClass(cookie)
			if len(ls) != 0 {
				for _,element := range ls {

					nameToSend := "("+element.kch+")"+element.kcmc+" - "+element.xf+" 学分"
					detailResult := getDetail(element.kch_id,cookie)
					for _,detail := range detailResult {
						chooseClass(detail.jxb_ids,element.kch_id,nameToSend,cookie)
...
func doTaskSequential(cookie Cookie) {
